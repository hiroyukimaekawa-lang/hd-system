...
function normalizeSystemGenre(genre, searchGenre, sourceGenre, storeName) {
  ...
  // 検索に使ったジャンル語（検索ジャンル列）を最後のフォールバックとして使う。
  // （Googleマップ拡張機能のように、検索した時点のジャンルが明確にわかっている場合に有効）
  if (!isValidHdGenre(mappedGenre) && textValue(searchGenre)) {
    const searchRaw = textValue(searchGenre);
    const searchMapped = HD_GENRE_MAP[searchRaw] || searchRaw;
    if (isValidHdGenre(searchMapped)) mappedGenre = searchMapped;
  }
